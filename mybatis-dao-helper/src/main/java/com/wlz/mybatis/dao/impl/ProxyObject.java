...
public class ProxyObject implements MethodInterceptor {
	...
	public ProxyObject(SelectContext selectContext) {
		...
		if (methodName.startsWith("get") && returnType != null && !Toolkit.isBasicType(returnType)) {
			...
			if (Toolkit.isListType(returnType)) {
				...
				if (many != null) {
					...
					SelectContext context = new SelectContext(clazz, selectContext.getDaoConfig(), selectContext.getDaoMapper(), Cnd.where(many.getManyIdFieldName(), "=", idObj));
				...
				ICondation cnd = Cnd.where(table.getId().getId(), "=", idObj);
